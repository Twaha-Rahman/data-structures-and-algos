...
import { dfsRecursion } from "../dfs.ts";

Deno.test("Null as root node", () => {
  const result = dfsRecursion(null, []);
  assertEquals(result, []);
});
...
Deno.test("Test with BT Sample 00", () => {
  ...
  const result = dfsRecursion(dfs_test_00, []);
...
Deno.test("Test with BT Sample 01", () => {
  ...
  const result = dfsRecursion(dfs_test_01, []);
...
Deno.test("Test with BT Sample 02", () => {
  ...
  const result = dfsRecursion(dfs_test_02, []);
...
Deno.test("Test with BT Sample 03", () => {
  ...
  const result = dfsRecursion(dfs_test_03, []);
